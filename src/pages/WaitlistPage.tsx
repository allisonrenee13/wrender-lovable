...
function EmailCapture({ onSubmit, email, setEmail, state }: {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col sm:flex-row items-center gap-3 w-full max-w-md">
      <input
        type="email"
        required
    ...
    <div id="landing-scroll" className="h-screen overflow-y-auto bg-background text-foreground">
      ...
      {mobileMenuOpen && (
        <div className="sm:hidden fixed inset-x-0 top-[60px] z-40 bg-background border-b border-border px-6 py-4 flex flex-col gap-3">
          ...
          <Button onClick={() => { document.getElementById("landing-scroll")?.scrollTo({ top: 0, behavior: "smooth" }); setMobileMenuOpen(false); }} className="bg-primary text-primary-foreground text-sm rounded-full w-full h-9">Get early access</Button>
        </div>
      )}
      ...
      <section id="about" className="py-20 px-6 md:px-12 bg-[hsl(40,20%,97%)]">
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {["A precise line sketch of your setting, always open beside your writing","Every location planned before you write the scene","Plot events pinned to the sketch so you can see the story spatially","Characters tracked across your world so you always know where everyone is","Visual clarity as you write"].map((t) => (
      ...
      <section className="py-20 px-6 md:px-12 bg-[hsl(40,20%,97%)]">
        <div className="max-w-5xl mx-auto text-center mb-12">
          ...
          <p className="text-muted-foreground max-w-lg mx-auto">Whether your world is a fantasy kingdom, a controlled dystopia, or a sun-bleached island, if place shapes your story, it helps to see it.</p>
        </div>
      ...
      <section className="py-24 px-6 text-center bg-[hsl(40,20%,97%)]">
        <h2 className="font-serif text-3xl md:text-5xl font-semibold text-foreground mb-4">Be first to Wrender your world.</h2>
